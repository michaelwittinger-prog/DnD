 ...
 *   1. validateGameState(state) — JSON Schema validation via Ajv
 *   2. validateInvariants(state) — 25 game-logic invariants
 *   3. validateAll(state) — runs both, merges errors
 *
 * Usage:
 *   node src/state/validateGameState.mjs          # runs self-test
 *   import { validateGameState, validateInvariants, validateAll } from './validateGameState.mjs'
 */
...
const typesSchema = JSON.parse(readFileSync(resolve(ROOT, "schemas/mir_types.schema.json"), "utf-8"));
const gameStateSchema = JSON.parse(readFileSync(resolve(ROOT, "schemas/mir_gamestate.schema.json"), "utf-8"));

const ajv = new Ajv({ allErrors: true, strict: false, validateSchema: false });
addFormats(ajv);

// Register types schema so $ref "mir_types.schema.json#/$defs/..." resolves
ajv.addSchema(typesSchema, "mir_types.schema.json");
const compiledValidate = ajv.compile(gameStateSchema);

// ── Schema validation ──────────────────────────────────────────────────

/**
 * Validate a GameState against the JSON Schema only.
 * @param {object} state
...
/**
 * Validate all 25 MIR invariants (docs/mir_state_invariants.md).
...
export function validateInvariants(state) {
  ...
  // Build blocked set (uses blocksMovement flag now)
...
export function validateAll(state) {
  ...
  const ir = validateInvariants(state);
  return ir;
}
...
if (isMain) {
  ...
  console.log("[1] Exploration example — fully valid");
  { const r = validateAll(explorationExample); check(r.ok, "validateAll passes"); if (!r.ok) r.errors.forEach(e => console.log(`     ⚠ ${e}`)); }

  console.log("\n[2] Combat example — fully valid");
  { const r = validateAll(combatExample); check(r.ok, "validateAll passes"); if (!r.ok) r.errors.forEach(e => console.log(`     ⚠ ${e}`)); }

  console.log("\n[3] Invalid example — schema passes, invariants fail");
  {
    const sr = validateGameState(invalidExample);
    check(sr.ok, "schema passes (structure valid)");
    const ir = validateInvariants(invalidExample);
    check(!ir.ok, "invariants fail");
    check(ir.errors.some(e => e.includes("UNIQUE_ENTITY_IDS")), "UNIQUE_ENTITY_IDS detected");
    check(ir.errors.some(e => e.includes("POSITION_IN_BOUNDS")), "POSITION_IN_BOUNDS detected");
    check(ir.errors.some(e => e.includes("NO_ENTITY_ON_BLOCKED")), "NO_ENTITY_ON_BLOCKED detected");
    check(ir.errors.some(e => e.includes("COMBAT_ACTIVE_ENTITY_EXISTS")), "COMBAT_ACTIVE_ENTITY_EXISTS detected");
    check(ir.errors.some(e => e.includes("COMBAT_ROUND_POSITIVE")), "COMBAT_ROUND_POSITIVE detected");
    check(ir.errors.some(e => e.includes("UI_SELECTED_ENTITY_EXISTS")), "UI_SELECTED_ENTITY_EXISTS detected");
    check(ir.errors.some(e => e.includes("UI_HOVERED_CELL_IN_BOUNDS")), "UI_HOVERED_CELL_IN_BOUNDS detected");
    console.log(`     (${ir.errors.length} invariant violations)`);
  }

  console.log("\n[4] validateAll on invalid → merged errors");
  { const r = validateAll(invalidExample); check(!r.ok, "validateAll fails"); check(r.errors.length > 0, `${r.errors.length} errors`); }

  console.log(`\n══════════════════════════════════════════════════`);
  console.log(`Results: ${passed}/${passed + failed} passed, ${failed} failed`);
  console.log(failed === 0 ? "PASS: all MIR GameState validator tests passed" : "FAIL: some tests failed");
